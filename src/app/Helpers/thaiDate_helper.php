...
if (!function_exists('vhidToAddress')) {
    /**
     * แปลงรหัส vhid (8 หลัก) เป็นข้อความที่อยู่แบบเต็ม
     * @param string $vhid
     * @return string
     */
    function vhidToAddress(string $vhid): string
    {
        if (strlen($vhid) !== 8) {
            return '';
        }

        $changwatCode = substr($vhid, 0, 2);
        $ampurCode    = substr($vhid, 0, 4);
        $tambonCode   = substr($vhid, 0, 6);
        $villageCode  = $vhid;

        // โหลด Model
        $changwatModel = new \App\Models\CchangwatModel();
        $ampurModel    = new \App\Models\CampurModel();
        $tambonModel   = new \App\Models\CtambonModel();
        $villageModel  = new \App\Models\CvillageModel();

        $changwat = $changwatModel->find($changwatCode);
        $ampur    = $ampurModel->find($ampurCode);
        $tambon   = $tambonModel->find($tambonCode);
        $village  = $villageModel->find($villageCode);

        $changwatName = $changwat['changwatname'] ?? '';
        $ampurName    = $ampur['ampurname'] ?? '';
        $tambonName   = $tambon['tambonname'] ?? '';
        $villageName  = $village['villagename'] ?? '';

        return "หมู่บ้าน{$villageName} ต.{$tambonName} อ.{$ampurName} จ.{$changwatName}";
    }
}